...
namespace Injector
{
    public class MethodInjector
    {
        ...
        {
            this.InjectBefore(sourceTypeName, sourceMethodName, targetTypeName, targetMethodName, 0, includeCallingObject, includeArgumentCount, passArgumentsByRef);
        }

        public void InjectAsFirst(string sourceTypeName, string sourceMethodName, MethodBody targetMethodBody,
            bool includeCallingObject = false, int includeArgumentCount = 0, bool passArgumentsByRef = false)
        {
            this.InjectBefore(sourceTypeName, sourceMethodName, targetMethodBody, targetMethodBody.Instructions.First(),
        ...
        {
            MethodBody targetMethodBody = CecilHelper.GetMethodDefinition(this._targetModule, targetTypeName, targetMethodName, useFullName).Body;
            Instruction instruction = targetMethodBody.Instructions[instructionIndex];

            this.InjectBefore(sourceTypeName, sourceMethodName, targetMethodBody, instruction, includeCallingObject, includeArgumentCount, passArgumentsByRef, useFullName);
        }
        ...
        {
            MethodDefinition sourceMethod = CecilHelper.GetMethodDefinition(this._sourceModule, sourceTypeName, sourceMethodName, useFullName);
            MethodReference sourceMethodReference = CecilHelper.GetMethodReference(this._targetModule, sourceMethod);

            this.InjectBefore(sourceMethodReference, targetMethodBody, targetInstruction, includeCallingObject, includeArgumentCount, passArgumentsByRef);
        }
            ...
            Instruction targetInstruction, bool includeCallingObject = false, int includeArgumentCount = 0, bool passArgumentsByRef = false)
        {
            ...
            if (includeArgumentCount > 0)
            {
                ...
                {
                    Instruction argumentInstruction = Instruction.Create(argumentOpCode, targetMethodBody.Method.Parameters[i - 1]);
